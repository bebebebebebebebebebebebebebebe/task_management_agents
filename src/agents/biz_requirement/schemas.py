...
class Stakeholder(BaseModel):
    ...
    expectations: str = Field(
        ..., description="ステークホルダーが本プロジェクトに求める成果や価値"
    )
...
class ProjectGoal(BaseModel):
    ...
    rationale: str = Field(
        ..., description="プロジェクトの目的や目標を達成するための根拠や理由"
    )
...
class ScopeItem(BaseModel):
    ...
    in_scope: str = Field(
        ..., description="プロジェクトの範囲内に含まれるもの（例：「○○の開発」）"
    )
    out_of_scope: str = Field(
        ..., description="プロジェクトに『含めない』機能・範囲（除外対象）"
    )


class NonFunctionalRequirement(BaseModel):
    category: str = Field(
        ..., description="非機能要件カテゴリ（性能, セキュリティ, 可用性, 運用性 など）"
    )
...
class Constraint(BaseModel):
    description: str = Field(
        ..., description="制約事項（例：既存システム連携必須、法規制、予算上限など）"
    )
...
class Schedule(BaseModel):
    ...
    milestones: Optional[List[str]] = Field(
        None, description="主要マイルストーン一覧（任意）"
    )
...
class ProjectBusinessRequirement(BaseModel):
    ...
    project_name: Optional[str] = Field(
        None, description="プロジェクトまたはプロダクトの正式名称"
    )
    description: Optional[str] = Field(None, description="プロジェクトの概要や目的")
    background: Optional[str] = Field(
        None, description="プロジェクト開始の背景や現状の課題"
    )
    goals: Optional[List[ProjectGoal]] = Field(
        None, description="プロジェクト目標の一覧"
    )
    stake_holders: Optional[List[Stakeholder]] = Field(
        None, description="プロジェクトに関わる利害関係者の一覧"
    )
    scopes: Optional[List[ScopeItem]] = Field(
        None, description="プロジェクトの範囲（スコープ）の定義"
    )
    constraints: Optional[List[Constraint]] = Field(
        None, description="予算・技術・法規制などの制約事項"
    )
    non_functional: Optional[List[NonFunctionalRequirement]] = Field(
        None, description="性能・セキュリティ等の非機能要件"
    )
...
class OutlineItem(BaseModel):
    ...
    headings: List[str] = Field(
        default_factory=list, description="その章に含まれる主要な見出しのリスト"
    )
    section_summary: Optional[str] = Field(
        None, description="このセクションでカバーすべき内容の簡潔な要約"
    )
...
class DynamicOutline(BaseModel):
    ...
    suggested_outline: List[OutlineItem] = Field(
        ..., description="提案されたアウトライン構造"
    )
    thought_process: str = Field(
        ..., description="そのアウトラインを提案した思考プロセスや理由"
    )
...
class DetailedSectionContent(BaseModel):
    ...
    thought_process: str = Field(
        ..., description="その内容を記述した思考プロセスや理由"
    )
...
class RequirementState(MessagesState):
    ...
    detailed_sections: Optional[List[DetailedSectionContent]] = Field(
