...
def get_missing_fields(requirement: ProjectBusinessRequirement | None) -> list[str]:
...
def get_missing_mandatory_fields(
    requirement: ProjectBusinessRequirement | None,
) -> list[str]:
    if requirement is None:
        return MANDATORY
...
def generate_ai_response_complete_madatory_case(
    is_asked_for_optional: bool,
    missing_optional_fields: list[str],
) -> str:
    """必須項目が揃った場合の応答を生成"""
    # current_requirement = state.get("current_requirement") or ProjectBusinessRequirement()
    # is_user_responded = state.get("messages", []) and isinstance(state['messages'][-1], HumanMessage)
    if missing_optional_fields and is_asked_for_optional:  # 任意項目が不足している場合
        optional_questions = build_questions(missing_optional_fields[:2])
        return f"""
...
def generate_ai_response_incomplete_mandatory_case(
    is_user_responded: bool,
    missing_mandatory_fields: list[str],
) -> str:
    """必須項目が不足している場合の応答を生成"""
    # current_requirement = state.get("current_requirement") or ProjectBusinessRequirement()
    # is_user_responded = state.get("messages", []) and isinstance(state['messages'][-1], HumanMessage)
    questions_to_ask = build_questions(missing_mandatory_fields)
...
def followup_node(state: RequirementState) -> RequirementState:
    ...
    special_command_result = handle_special_commands(state, user_message)
...
def detail_generation_node(state: RequirementState) -> RequirementState:
    ...
    for section in dynamic_outline.suggested_outline:
        section_future = detail_generation_task(
